...
func RenderMain(c *gin.Context, cfg conf.Config, funcMap template.FuncMap, searchPattern string) (bytes.Buffer, error) {
	client := &http.Client{
	...
	if resp.StatusCode != 200 {
		log.Printf("Non 200 Response: %v", resp)
	...
	tmpl, err := template.
		New("helper_main.tmpl").
		Funcs(funcMap).
		ParseFiles("templates/helper_main.tmpl")
	...
	data := map[string]interface{}{
		...
		"Navigation": `Page renders example`,
